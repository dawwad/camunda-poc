...
public class DummyData {
    ...
    private static final Logger LOGGER = Logger.getLogger(DummyData.class.getName());;

    public static void createUsers(ProcessEngine processEngine){
...
//        authorizationService.saveAuthorization(taskMaryAuth);
        ...
        filterProperties.clear();
        filterProperties.put("description", "Tasks for Group Sales");
        filterProperties.put("priority", -3);
        addVariables(filterProperties);
        query = taskService.createTaskQuery().taskCandidateGroupIn(Arrays.asList("sales")).taskUnassigned();
        Filter candidateGroupTasksFilteSales = filterService.newTaskFilter().setName("Sales").setProperties(filterProperties).setOwner("demo").setQuery(query);
        filterService.saveFilter(candidateGroupTasksFilteSales);


        Authorization managementGroupFilterReadSales = authorizationService.createNewAuthorization(Authorization.AUTH_TYPE_GRANT);
        managementGroupFilterReadSales.setResource(FILTER);
        managementGroupFilterReadSales.setResourceId(candidateGroupTasksFilteSales.getId());
        managementGroupFilterReadSales.addPermission(READ);
        managementGroupFilterReadSales.setGroupId("sales");
        authorizationService.saveAuthorization(managementGroupFilterReadSales);
        ...
        Filter candidateGroupTasksFilterAccounting = filterService.newTaskFilter().setName("Accounting").setProperties(filterProperties).setOwner("demo").setQuery(query);
        filterService.saveFilter(candidateGroupTasksFilterAccounting);

        Authorization managementGroupFilterReadAccounting = authorizationService.createNewAuthorization(Authorization.AUTH_TYPE_GRANT);
        managementGroupFilterReadAccounting.setResource(FILTER);
        managementGroupFilterReadAccounting.setResourceId(candidateGroupTasksFilteSales.getId());
        managementGroupFilterReadAccounting.addPermission(READ);
        managementGroupFilterReadAccounting.setGroupId("Accounting");
        authorizationService.saveAuthorization(managementGroupFilterReadAccounting);
    ...
    protected static void addVariables(Map<String, Object> filterProperties) {
    ...
    protected static void addVariable(List<Object> variables, String name, String label) {
